...
PYEOF
  ...
  steps {
    script {
      sh "echo 'npm ci --prefer-offline --no-audit && npm run lint:types' > ${WORKSPACE}/lint.sh"
      sh """
        docker run --rm \
          -v ${WORKSPACE}:/app \
          -w /app \
...
stage('Tests') {
  steps {
    script {
      sh "echo 'npm ci --prefer-offline --no-audit && npm test -- --runInBand --forceExit' > ${WORKSPACE}/test.sh"
      sh """
        docker run --rm \
          -v ${WORKSPACE}:/app \
          -w /app \
