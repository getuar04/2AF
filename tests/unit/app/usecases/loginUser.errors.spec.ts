...
describe("LoginUser - error cases", () => {
  const mockUserRepository: jest.Mocked<UserRepository> = {
    findById: jest.fn(), findByEmail: jest.fn(), create: jest.fn(), enableTwoFactor: jest.fn(),
  };
  const mockPasswordHasher: jest.Mocked<PasswordHasher> = { hash: jest.fn(), compare: jest.fn() };
  const mockTokenProvider: jest.Mocked<TokenProvider> = {
    generateAccessToken: jest.fn(), generateRefreshToken: jest.fn(), verifyRefreshToken: jest.fn(),
  };
  const mockCacheProvider: jest.Mocked<CacheProvider> = { set: jest.fn(), get: jest.fn(), delete: jest.fn() };
  const mockAuditRepository: jest.Mocked<AuthAuditRepository> = { create: jest.fn(), findAll: jest.fn() };
  const mockEventBus: jest.Mocked<EventBus> = { publish: jest.fn() };
  const mockIdGenerator: jest.Mocked<IdGenerator> = { generate: jest.fn().mockReturnValue("id") };

  const makeUseCase = () => new LoginUser(
  ...
  it("should throw INVALID_CREDENTIALS when user not found", async () => {
    ...
    try {
      await makeUseCase().execute({ email: "notfound@test.com", password: "Password123" });
  ...
  it("should throw INVALID_CREDENTIALS when password is wrong", async () => {
    ...
    try {
      await makeUseCase().execute({ email: "user@test.com", password: "wrongpassword" });
      fail("should have thrown");
    } catch (e) {
      expect(e).toBeInstanceOf(AppError);
      expect((e as AppError).code).toBe("INVALID_CREDENTIALS");
  ...
  it("should throw INVALID_EMAIL for bad email format", async () => {
    await expect(makeUseCase().execute({ email: "bademail", password: "Password123" }))
      .rejects.toThrow(AppError);
  });

  it("should throw PASSWORD_REQUIRED for empty password", async () => {
    await expect(makeUseCase().execute({ email: "user@test.com", password: "" }))
      .rejects.toThrow(AppError);
  });

  it("should not call token provider when credentials are invalid", async () => {
    mockUserRepository.findByEmail.mockResolvedValue(null);
    try { await makeUseCase().execute({ email: "bad@test.com", password: "Pass123" }); } catch {}
  ...
  it("should cache challenge when 2FA is required", async () => {
    mockUserRepository.findByEmail.mockResolvedValue({
      id: "user-1", fullName: "Test", email: "user@test.com",
      passwordHash: "hash", role: "user", isTwoFactorEnabled: true,
      twoFactorSecret: "SECRET", createdAt: new Date(), updatedAt: new Date(),
    });
    mockPasswordHasher.compare.mockResolvedValue(true);
    mockIdGenerator.generate.mockReturnValueOnce("challenge-id");

    const { output } = await makeUseCase().execute({ email: "user@test.com", password: "Password123" });
